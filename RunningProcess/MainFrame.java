...
public class MainFrame extends JFrame implements ListSelectionListener {
	...
	JTextField selectedList = new JTextField(20);
	JLabel Label = new JLabel();
	JLabel addLabel = new JLabel();
	JLabel addPrcs = new JLabel();
	...
	public MainFrame()
	{
		...
		selectedList.setEditable(true); //텍스트 필드 수정 불가상태

		//컴포넌트에 기능 추가
		listPanel.add(scroll); //리스트 컴포넌트에 스크롤 기능 추가
		labelPanel.add(Label); //선택한 프로세스를 표시할 라벨 부착
		
		addPrcs.setText("등록된 프로세스 : ");
		Label.setText("선택 프로세스 : ");
		Label.add(selectedList);
		
		addLabel.setText(Kill.PRCS);
	...
	if(!e.getValueIsAdjusting()) //마우스가 버튼 클릭 후 떼면 리스트에서 프로세스명 선택
	{
		...
		selectedList.setText((String) list.getSelectedValue());
	}
	}
